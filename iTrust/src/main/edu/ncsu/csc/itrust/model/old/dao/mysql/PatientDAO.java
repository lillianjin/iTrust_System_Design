...
public class PatientDAO {
	...
	public boolean isValidInfant(long mid) throws DBException {
		...
		try {
			...
			ps = conn.prepareStatement("SELECT DateOfBirth FROM patients WHERE MID = ?");
			ps.setLong(1,mid);
			ResultSet rs = ps.executeQuery();
			if(rs.next()) {
				Date curr = new Date();
				java.sql.Date dbSqlDate = rs.getDate("DateOfBirth");
				Date birth = new java.util.Date(dbSqlDate.getTime());
				long Days = (curr.getTime() - birth.getTime()) / 1000 / 24 / 60 / 60;
				if (Days <= 365)
					return true;
			}
			rs.close();
